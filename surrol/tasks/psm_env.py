...
class PsmEnv(SurRoLGoalEnv):
    ...
    def compute_reward(self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info):
        ...
        # return self._is_success(achieved_goal, desired_goal).astype(np.float32) - 0.98-(0.1*d)
        # return self._is_success(achieved_goal, desired_goal).astype(np.float32) -(5 * d)
    ...
    def _env_setup(self):
        ...
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'sphere/sphere.urdf'),
                            globalScaling=self.SCALING)
        p.changeVisualShape(obj_id, -1, rgbaColor=[0, 0, 0, 0])
        ...
        # self.actions = []
        # self._view_matrix = p.computeViewMatrixFromYawPitchRoll(
        #     cameraTargetPosition=((-0.05 + 0.01 * np.random.randn()) * self.SCALING, np.clip(0.01 * np.random.randn(),0,0.01),
        #                           (0.375 + 0.01 * np.random.randn()) * self.SCALING),
        #     distance=(0.81 + 0.01 * np.random.randn()) * self.SCALING,
        #     yaw=90 + 0.01 * np.random.randn(),
        #     pitch=-30 + 0.01 * np.random.randn(),
        #     roll=0 + 0.01 * np.random.randn(),
        #     upAxisIndex=2
        # )
